...
import os
from dataset.cityscapes_dataloader import CityscapesDataLoader 
...
def main(config: DictConfig):
    ...
    logger = TensorBoardLogger("tb_logs", name="Cityscapes_model")
    model = DepthSegmentAnythingV2(**{**model_configs[config.trainer.encoder], 'max_depth': config.trainer.max_depth})
    model.load_original_weights_for_training(config.trainer.original_weights, False)
    lr_monitor = LearningRateMonitor(logging_interval='step')

    max_iou_callback = ModelCheckpoint(
        dirpath=config.trainer.ckpt_output_path,
        filename='unfrozen_netork-segmentation_DepthAnythingV2_halfsize-train_with-random_crops_{epoch}-{train_loss_epoch:.2f}-{val_loss_epoch:.2f}-{train_jaccard_epoch:.2f}-{val_jaccard_epoch: .2f}',
        save_top_k=1,
        mode="max",
        monitor="val_jaccard",
        every_n_epochs=1, 
